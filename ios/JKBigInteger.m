...
- (id)lcm:(JKBigInteger *)b {
    return [[[self abs] divide:[b gcd:self]] multiply:[b abs]];
}

+ (id)egcd:(JKBigInteger*)a b:(JKBigInteger*)b x:(JKBigInteger**)x y:(JKBigInteger**)y {
    JKBigInteger *ZERO = [[JKBigInteger alloc] initWithUnsignedLong:0];
    JKBigInteger *ONE = [[JKBigInteger alloc] initWithUnsignedLong:1];
    if ([a compare:ZERO] ==0) {
        *x = ZERO;
        *y = ONE;
        return b;
    ...
    JKBigInteger *x1;
    JKBigInteger *y1;
    JKBigInteger *gcd= [JKBigInteger egcd:[b remainder:a] b:a x:&x1 y:&y1];
    *x = [y1 subtract:[[b divide:a] multiply:x1]];
    *y = x1;
    return gcd;
}

- (id)modInverse:(JKBigInteger *)mod {
    JKBigInteger *ONE = [[JKBigInteger alloc] initWithUnsignedLong:1];
    JKBigInteger *x,*y;
    JKBigInteger *g = [JKBigInteger egcd:self b:mod x:&x y:&y];
    if ([g compare:ONE] != 0) {
        NSLog(@"Inverse doesn't exist");
        return NULL;
    }
    return [[[x remainder:mod] add:mod] remainder:mod];
}
- (id)sqrt {
    JKBigInteger *ZERO = [[JKBigInteger alloc] initWithUnsignedLong:0];
    JKBigInteger *ONE = [[JKBigInteger alloc] initWithUnsignedLong:1];
    ...
    int shift = 2;
    JKBigInteger *shifted = [self shiftRight:shift];
    while (([shifted compare:ZERO] != 0) && ([shifted compare:self] != 0)) {
        shift += 2;
        shifted = [self shiftRight:shift];
    }
    
    JKBigInteger *result = ZERO;
    while (shift >= 0) {
        result = [result shiftLeft:1];
        JKBigInteger *candidate = [result add:ONE];
        JKBigInteger *squared = [candidate pow:2];
        if ([squared compare:[self shiftRight:shift]] <= 0) {
            result = candidate;
        }
        shift -= 2;
    }
...
- (int)getLowestSetBit {
    JKBigInteger *ONE = [[JKBigInteger alloc] initWithUnsignedLong:1];
    for (int i=0; i<[self getBitLength]; i++) {
        if ([[[self shiftRight:i] bitwiseAnd:ONE] compare:ONE] == 0) {
            return i;
        }
    }
    NSLog(@"Error: candidate has no lowest set bit(%d) - %@", [self getBitLength], [self stringValueWithRadix:16]);
    assert(false);
    return -1;
}
...
- (int)getBitLength {
    JKBigInteger *ZERO = [[JKBigInteger alloc] initWithUnsignedLong:0];
    int numBitsMin = [self countBytes]*8-7;
    for (int i=numBitsMin; i<numBitsMin+8; i++) {
        if ([[self shiftRight:i] compare:ZERO] == 0) {
            return i;
        }
    }
    return 0;
